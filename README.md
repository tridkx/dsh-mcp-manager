...
| **按需环境说明（notes）** | 每台服务器可写一段 `notes`（如「Blender 不在 PATH」「9877 端口约定」「侧边栏显示 bug」）。**完整内容不进每个请求**，只在模型 `describe` 该服务器某个工具时随附 |
| **首屏清单（可发现性）** | 只有工具 schema 是不够的：lazy 下模型只看到一个匿名网关，不知道有哪几台服务器，于是永远不会去 `describe`，`notes` 也就形同不存在。插件因此注册一个 system prompt section，列出**服务器名 + `mode` + `notes` 首行摘要**（上限 700 字符），并说明用 `mcp_tools` 取工具与完整说明 |
...
版本：**1.2.3**（v1.2.3 修**可发现性**：插件此前只注册工具、从不注入提示词，导致 lazy 下模型不知道有哪几台 MCP 服务器，永远不会 `describe`，`notes` 形同不存在——实机上表现为「AI 不知道 Blender 装在哪」。现注册 system prompt section，注入服务器名/mode/`notes` 首行摘要（≤700 字符）；同时修 `describe` 用 `brief()` 把多行 `notes` 压成单行的 bug（改用保留结构的 `noteText()`），并让 `mcp_tools list` 标出「✎有环境说明」。v1.2.2 修两处：①**子进程清理改为等待**——`terminate()` 返回的清理 promise 过去被丢弃，插件重载/dsh 重启时异步收尾跑不完，`cmd.exe → uvx → MCP 服务器` 会变成孤儿进程；②`mcp_tools load` 改为**只作用于当前进程**，不再把 `eager` 写进配置，重载插件即回到 lazy。v1.2.1 工具列表改为**整代原子替换**；v1.2.0 新增 lazy 按需注入 + `mcp_tools` 网关、服务器 `notes` 按需说明、健康探针与诚实的连接状态；v1.1.0 修复「编辑页改名实为报错+删除」问题）。
许可：MIT。
