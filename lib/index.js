...
export async function apply(ctx) {
  ...
  function makeEntry(cfg) {
    return { config: cfg, state: 'disconnected', error: null, handle: undefined, transport: undefined, tools: [], disposers: [], loaded: new Set(), health: null };
  }
  ...
  function registerTools(entry) {
    unregisterTools(entry);
    if (entry.config.mode === 'off') return;
    const disposers = [];
    ...
    // lazy: the server's own tools stay unregistered; `mcp_tools` is the single
    // stable entry point and is registered once per plugin apply().
    entry.disposers = disposers;
  }
  ...
  // through; image blocks keep their native form so a vision-capable model
  // receives the real picture instead of the old `[image]` placeholder.
    ...
    if (Array.isArray(result && result.content)) {
      for (const block of result.content) {
        ...
        if (block.type === 'image') {
          ...
          if (mime && data && data.length <= 4 * 1024 * 1024 && !/\s/.test(data)) blocks.push({ type: 'image', source: { type: 'base64', media_type: mime, data } });
          else blocks.push({ type: 'text', text: '[image unavailable: ' + (block.mimeType || 'unknown media type') + (mime ? (data.length > 4 * 1024 * 1024 ? '; too large to inline' : '; empty data') : '; not a supported raster format') + ']' });
          continue;
        }
  ...
  function blocksToText(blocks) {
    return blocks.map((b) => (b.type === 'text' ? b.text : '[图片: ' + ((b.source && b.source.media_type) || 'image') + ']')).join('\n');
  }
  async function gatewayCall(entry, rawName, args) {
    const result = await callToolRaw(entry, rawName, args || {});
    const blocks = contentBlocksOf(result);
  ...
  async function runGateway(args) {
    ...
    if (action === 'describe' || action === 'call') {
      ...
      entry.loaded.add(toolName);
      return await gatewayCall(entry, toolName, args.arguments || {});
    }
    if (action === 'load') {
