...
// `systemPrompt` is deliberately NOT in this list: the prompt section below is
// an enhancement, not a hard dependency, so it is read with ctx.get() and the
// plugin still mounts (tool bridging + GUI intact) where that service is absent.
...
export async function apply(ctx) {
  ...
  function makeEntry(cfg) {
    // `persistedMode` is the mode stored on disk; `config.mode` is the live
    // one. They differ only while a `mcp_tools load` promotion is in effect.
    return { config: cfg, persistedMode: cfg.mode, state: 'disconnected', error: null, handle: undefined, transport: undefined, tools: [], disposers: [], loaded: new Set(), health: null, publicNames: [], generation: { status: 'empty', replacedAt: 0, toolCount: 0, problems: [], reason: null }, resyncing: false, resyncQueued: false, generator: null };
  }
  ...
  async function saveServers() {
    try {
      ...
      for (const entry of servers.values()) {
        // Persist the mode the user chose, not a transient `mcp_tools load`
        // promotion: that promotion is documented as "a plugin reload returns
        // to lazy", so writing it here would silently make it permanent and
        // defeat the reload it promises. An explicit mode change goes through
        // updateServer, which refreshes `persistedMode` first.
        arr.push(Object.assign({}, entry.config, { mode: entry.persistedMode || entry.config.mode }));
      }
  ...
  /**
   * Tear one connection down and WAIT for the child's managed range to become
   * empty before releasing the handle.
   *
   * The subprocess seam's `terminate()` is asynchronous (it stages
   * SIGTERM→SIGKILL on a POSIX range, or terminates and then polls a Windows
   * Job Object to quiescence) and its `void` return type only means "the
   * procedure started". Dropping the returned promise leaves this function
   * synchronous, so a caller that exits right after it — the plugin-unload
   * effect during an `dsh` restart, or a reload after an edit — would finish
   * its own teardown while `cmd.exe` → `uvx` → the MCP server were still
   * alive, orphaning the whole chain. Awaiting it makes unload a real
   * guarantee: when the effect resolves, no child of this entry outlives it.
   *
   * The timeout is a backstop only: a wedged handle must not make unload hang.
   * The range is already condemned when the wait starts, so what is waited on
   * is the provider's own escalation, normally a few hundred milliseconds.
   */
  const TEARDOWN_WAIT_MS = 8000;
  async function teardownConnection(entry) {
    ...
    try {
      // Typed `void`, but every implementation returns its cleanup promise;
      // capturing it here is what makes the wait below possible.
      termination = handle.terminate();
    } catch (e) {
      console.error('[mcp-manager] terminate failed for ' + entry.config.name + ': ' + ((e && e.message) || e));
    }
    if (!termination || typeof termination.then !== 'function') return;
    try {
      await Promise.race([
        termination,
        new Promise((resolve) => ctx.timeout(resolve, TEARDOWN_WAIT_MS)),
      ]);
    } catch (e) {
      console.error('[mcp-manager] termination wait failed for ' + entry.config.name + ': ' + ((e && e.message) || e));
    }
  }
  ...
  async function disconnect(entryName) {
    ...
    if (!entry) throw new Error('server "' + entryName + '" not found');
    if (entry.state === 'connected' || entry.state === 'connecting') {
      // Awaited: the caller may replace this entry's config immediately
      // afterwards (update/rename/remove), and a still-draining range would
      // then belong to a handle nobody holds any more.
      await teardownConnection(entry);
      entry.state = 'disconnected';
      entry.error = null;
    }
  ...
  function swapToolsGeneration(entry, ownsRecord) {
    const next = { status: 'applied', replacedAt: Date.now(), toolCount: entry.config.mode === 'eager' ? entry.tools.length : 0, problems: [], reason: null };
    ...
    if (entry.config.mode !== 'eager') {
      ...
      // `toolCount` counts REGISTERED tools here, so it must read 0 rather than
      // the number of tools discovered — the old value made a lazy server with
      // 31 discovered tools report `toolCount: 31`, i.e. look like it was
      // injecting schemas into every request when it was injecting none.
      unregisterTools(entry);
      entry.publicNames = [];
  ...
  function registerTools(entry) {
    if (entry.resyncing) {
      ...
      entry.generation = { status: 'queued', replacedAt: Date.now(), toolCount: entry.config.mode === 'eager' ? entry.tools.length : 0, problems: [], reason: null };
  ...
  /**
   * Bound a server's `notes` WITHOUT flattening it.
   *
   * `brief()` is for one-line tool descriptions and collapses every whitespace
   * run — correct there, destructive here: notes are the server's structured
   * environment knowledge (where the binary lives, which port, which startup
   * flags are forbidden), and folding them into a single line is what made a
   * 51-line Blender primer arrive as one unreadable block. Keeps paragraphs and
   * bullets, normalises CRLF, trims trailing spaces and blank-line runs.
   */
  function noteText(text, max) {
  ...
  function catalogText() {
    ...
    for (const entry of lazyServers()) {
      // Surface the existence of `notes` cheaply. Without this the model has no
      // signal that a server carries environment knowledge, so it never calls
      // describe and never learns where the binary lives — the injection then
      // looks like it is not happening at all.
      const hasNotes = entry.config.notes ? '  ✎有环境说明（describe 查看）' : '';
      const head = '## ' + entry.config.name + '  [' + entry.state + healthSuffix(entry) + ']' + hasNotes;
  ...
  function describeText(entry, tool) {
    ...
    if (notes) {
      parts.push('', '── 使用说明（本服务器配置的 notes，随 describe 注入） ' + '─'.repeat(8), noteText(notes, NOTES_MAX));
    }
    return parts.join('\n');
  }
  ...
  async function runGateway(args, exec) {
    ...
    if (action === 'load') {
      ...
      // Costs the full schema set per request once loaded, and is deliberately
      // IN-MEMORY ONLY — `persistedMode` is left alone so reloading the plugin
      // (or restarting dsh) restores the configured lazy mode, exactly as the
      // gateway description and README promise. Persisting it here would make
      // the promotion outlive the reload that is supposed to undo it.
      ...
      return {
        ...
        text: '已把 ' + serverName + ' 切到 eager：' + count + ' 个工具现在直接可调用。注意每个请求都会带上它们的 schema。' +
          '本次提升只作用于当前进程、不写入配置：重载插件或重启 dsh 后会恢复 lazy（磁盘上的 mode 仍是 ' + (entry.persistedMode || 'lazy') + '）。' +
          '要永久常驻请在配置里把该服务器的注入方式设为 eager。此前 describe 过的工具: ' + (before.length ? before.join(', ') : '（无）'),
      };
    }
  ...
  function catalogTextFor(entry) {
    ...
    const head = '## ' + entry.config.name + ' [' + entry.state + healthSuffix(entry) + ']'
      + (entry.config.notes ? '  ✎有环境说明（describe 查看）' : '');
    return head + '\n' + entry.tools.map((t) => {
  ...
  const gatewayTool = {
    ...
    description:
      ...
      '用 action:"call" 带 arguments 直接调用；用 action:"load" 可把某服务器临时提升为常驻注册（仅当前进程，重载后恢复按需）。\n' +
      '调用前先 describe 以拿到准确参数名。',
    parameters: {
      type: 'object',
      properties: {
        action: { type: 'string', enum: ['list', 'describe', 'call', 'load'], description: 'list: 工具目录；describe: 载入某工具的 schema + 服务器说明；call: 调用工具（arguments 必填）；load: 把服务器临时切到 eager 常驻注册（只改内存，重载插件或重启 dsh 后恢复配置里的模式）。' },
  ...
  async function updateServer(input) {
    ...
    existing.config = cfg;
    // An explicit configuration change states the mode to persist; it also
    // supersedes any in-flight `load` promotion on this entry.
    existing.persistedMode = cfg.mode;
  ...
  // ─────────────── first-screen prompt section (the discoverability fix) ───────────────
  /**
   * A dynamic system-prompt section naming the configured MCP servers.
   *
   * Tools alone are not discoverability. In lazy mode the model sees one
   * anonymous gateway (`mcp_tools`) plus `mcp_manager`, and a server's `notes`
   * — where the environment knowledge lives (e.g. where the Blender binary is)
   * — is only reachable by calling `describe`. A model that has not been told
   * the servers exist never makes that call, so the injection is invisible in
   * practice and the agent behaves as if MCP were not configured at all.
   *
   * This section fixes only that gap, and stays cheap by construction: one
   * line per server, only the FIRST LINE of `notes` (the abstract its author
   * writes to be read), capped globally. The full notes stay behind
   * `mcp_tools({action:"describe"})` — the two-tier design is unchanged.
   */
  const PROMPT_SUMMARY_MAX = 120;
  const PROMPT_TOTAL_MAX = 700;
  const PROMPT_SERVER_FLOOR = 90;
  function promptSectionText() {
    if (!servers.size) return '';
    // Per-server budget with a floor, so a loquacious first line on server #1
    // cannot silently truncate server #2 out of existence; the global cap
    // breaks ties when there are more servers than budget.
    const perServer = Math.max(PROMPT_SERVER_FLOOR, Math.floor(PROMPT_TOTAL_MAX / servers.size) - 40);
    const lines = [];
    for (const entry of servers.values()) {
      const mode = entry.config.mode;
      const notesLine = String(entry.config.notes || '').split('\n').map((l) => l.trim()).find(Boolean) || '';
      const summary = notesLine.length > perServer ? notesLine.slice(0, perServer - 1) + '…' : notesLine;
      lines.push('- ' + entry.config.name + '（' + mode + '）'
        + (summary ? '：' + summary : '')
        + (mode === 'lazy' ? '  〔工具与说明：mcp_tools list / describe〕' : ''));
    }
    // The point of this section is discoverability, so it must say the thing a
    // model gets wrong unaided: these tools are NOT in its tool list, and the
    // gateway is what reaches them.
    let text = 'MCP：本会话已配置以下 MCP 服务器（注意：这些服务器的工具**不在**你的工具列表里，'
      + 'lazy 模式下要用 `mcp_tools` 才能看到和调用）。\n'
      + lines.join('\n')
      + '\n`mcp_tools({action:"list"})` 列工具目录，`describe` 取参数 schema 与完整环境说明，'
      + '`call` 调用；`mcp_manager` 增删改服务器、连接/断开、健康探针。';
    if (text.length > PROMPT_TOTAL_MAX) {
      text = text.slice(0, PROMPT_TOTAL_MAX - 1) + '…';
    }
    return text;
  }
  ...
  if (systemPrompt && typeof systemPrompt.section === 'function') {
    ctx.effect(() => systemPrompt.section({
      name: 'mcp:servers',
      // Between the last repository tool section (TOOL_REPORT 2900) and
      // TOOLS_SDK (5000); owner-defined order, not a central slot name.
      order: 3000,
      text: () => promptSectionText(),
    }), 'mcp-manager: MCP server prompt section');
  }

  // ───────────────────── lifecycle ─────────────────────
  ctx.effect(() => () => {
    // Cordis awaits an async disposer (`_unload` → `runDisposable`), so
    // returning this promise is what keeps a dsh restart from orphaning every
    // stdio child chain; a synchronous loop of `terminate()` calls cannot.
    const closing = [];
    for (const entry of servers.values()) closing.push(teardownConnection(entry));
