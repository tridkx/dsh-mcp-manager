...
.mcp-dot[data-state=connected][data-health=degraded],.mcp-dot[data-state=connected][data-health=unknown]{background:var(--dsw-alias-state-warn-primary)}
.mcp-dot[data-state=connected][data-health=probing]{background:var(--dsw-alias-state-warn-primary);opacity:.6}
.mcp-badge[data-mode=lazy]{color:var(--dsw-alias-brand-primary);border-color:color-mix(in srgb,var(--dsw-alias-brand-primary) 45%,transparent)}
...
.mcp-cat-details div{word-break:break-all}`;
		...
		function McpManagerPage() {
			...
			const invalidateTools = (serverName) => setToolsCache((c) => {
				if (!(serverName in c)) return c;
			...
			const act = async (op, payload, key) => {
				...
				try {
					...
					if (payload && payload.name) invalidateTools(payload.name);
					await refresh();
				} catch (e) {
			...
			const openNew = () => setEditing({ originalName: '', name: '', transport: 'http', command: '', argsText: '', envText: '', cwd: '', url: '', headersText: '', toolCallTimeoutMs: '', mode: 'lazy', notes: '', healthTool: '', healthExpect: '' });
			const openEdit = (server) => {
				...
				setEditing({
					...
					mode: server.mode || 'lazy',
					notes: server.notes || '',
					healthTool: server.healthTool || '',
					healthExpect: server.healthExpect || '',
				});
			};
			...
			const saveForm = async () => {
				...
				const payload = {
					...
					mode: editing.mode || 'lazy',
					notes: editing.notes || '',
					healthTool: editing.healthTool || '',
					healthExpect: editing.healthExpect || '',
				};
			...
			const modeLabel = { lazy: '按需', eager: '常驻', off: '关闭' };
			const stateText = (server) => {
				const base = stateLabel[server.state] || server.state;
				const health = server.health;
				if (server.state !== 'connected') return base;
				if (!health || health.status === 'unchecked') return base + '（未校验后端）';
				if (health.status === 'healthy') return base + '（后端正常）';
				if (health.status === 'probing') return base + '（校验中）';
				if (health.status === 'degraded') return '服务不可用（后端未响应）';
				return base + '（健康状态未知）';
			};
			...
			return h('div', { className: 'mcp-page' },
				h('div', { className: 'mcp-toolbar' },
					...
					h('span', { className: 'mcp-sub' }, '（stdio / HTTP · 按需模式下工具通过 mcp_tools 网关取用；切到常驻则注册为 mcp__服务器__工具）'),
				...
				servers === null ? h('div', { className: 'mcp-empty' }, '加载中…') :
					servers.length === 0 ? h('div', { className: 'mcp-empty' }, '还没有 MCP 服务器。点击“+ 新增服务器”添加，例如 Godot AI 的 http://127.0.0.1:8000/mcp。') :
						servers.map((server) => h(ServerCard, {
							...
							onHealth: () => act('health', { name: server.name }, 'health:' + server.name),
							stateLabel,
							stateText,
							modeLabel,
						})),
				editing ? h('form', { className: 'mcp-form', onSubmit: (ev) => { ev.preventDefault(); saveForm(); } },
					...
					h('label', { className: 'mcp-field' }, '工具注入方式',
						h('select', { className: 'mcp-input', value: editing.mode || 'lazy', onChange: (ev) => setForm({ mode: ev.target.value }) },
							h('option', { value: 'lazy' }, '按需（默认）—— 工具不进每次请求，模型通过 mcp_tools 网关按需取用'),
							h('option', { value: 'eager' }, '常驻 —— 每个工具都直接注册，schema 计入每次请求'),
							h('option', { value: 'off' }, '关闭 —— 只管理连接，不向模型暴露任何工具'),
						)),
					h('label', { className: 'mcp-field' }, '使用说明 / notes（按需注入：模型 mcp_tools describe 某工具时随附）',
						h('textarea', { className: 'mcp-input mcp-textarea', placeholder: '把该服务器的环境知识写在这里，替代写进 AGENTS.md', value: editing.notes || '', onChange: (ev) => setForm({ notes: ev.target.value }) })),
					h('label', { className: 'mcp-field' }, '健康探针工具（留空 = 不校验后端；填 get_addon_status 这类只读工具）',
						h('input', { className: 'mcp-input', placeholder: '如 get_addon_status', value: editing.healthTool || '', onChange: (ev) => setForm({ healthTool: ev.target.value }) })),
					h('label', { className: 'mcp-field' }, '健康判据（探针返回里必须出现的文本；blender-mcp 把后端错误当成功返回，所以必须填）',
						h('input', { className: 'mcp-input', placeholder: '如 protocol_version', value: editing.healthExpect || '', onChange: (ev) => setForm({ healthExpect: ev.target.value }) })),
		...
		function ServerCard(props) {
			const { server, busy, expanded, tools, argsText, results, stateText, modeLabel, confirmRemove } = props;
			const toolBusyKey = (tool) => server.name + '|' + tool;
			const health = server.health;
			const loadedCount = (server.loaded || []).length;
			const meta = server.transport === 'http'
				? server.url
				: server.command + (server.args && server.args.length ? ' ' + server.args.map((a) => (a.indexOf(' ') >= 0 ? '"' + a + '"' : a)).join(' ') : '');
			const healthLine = server.state === 'connected' && health
				? health.status === 'healthy' ? '健康探针 ' + health.tool + ' 通过'
					: health.status === 'degraded' ? '健康探针 ' + health.tool + ' 失败：' + (health.error || '')
						: health.status === 'probing' ? '健康探针 ' + health.tool + ' 进行中…'
							: health.status === 'unchecked' ? '未配置健康探针：此“已连接”仅代表 MCP 握手成功，不代表后端可用'
								: '健康状态未知：' + (health.error || '')
				: null;
			const gen = server.generation;
			const genLine = gen && gen.status && gen.status !== 'empty' && gen.status !== 'applied'
				? (gen.status === 'queued' ? '工具代际：本次发布已交给在飞的重同步'
					: '工具代际被拒绝（' + (gen.status === 'keeping' ? '重同步拉取失败' : '新列表校验未通过') + '），仍在用上一代 '
						+ String(gen.toolCount) + ' 个工具：' + (gen.reason || ''))
				: null;
			return h('div', { className: 'mcp-card' },
				h('div', { className: 'mcp-row' },
					h('span', { className: 'mcp-dot', 'data-state': server.state, 'data-health': health ? health.status : undefined }),
					h('span', { className: 'mcp-name' }, server.name),
					h('span', { className: 'mcp-badge' }, server.transport),
					h('span', { className: 'mcp-badge', 'data-mode': server.mode }, modeLabel[server.mode] || server.mode || '按需'),
					h('span', { className: 'mcp-badge' }, stateText(server)),
					...
					server.healthTool ? h('button', { className: 'mcp-btn', disabled: !!busy, onClick: props.onHealth }, '体检') : null,
				...
				healthLine ? h('div', { className: health && health.status === 'degraded' ? 'mcp-err' : 'mcp-meta' }, healthLine) : null,
				genLine ? h('div', { className: 'mcp-err' }, genLine) : null,
				server.mode === 'lazy' && loadedCount ? h('div', { className: 'mcp-meta' }, '模型已 describe ' + String(loadedCount) + ' 个工具：' + (server.loaded || []).join(', ')) : null,
