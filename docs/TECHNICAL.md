...
- **首屏提示词注入（v1.2.3）**：`ctx.systemPrompt.section({ name:'mcp:servers', order:3000, text })`，`text` 是**动态函数**（每次组装重新求值，因此读到的是实时的服务器列表与状态）。`order: 3000` 落在最后一个仓库工具段 `TOOL_REPORT`(2900) 与 `TOOLS_SDK`(5000) 之间——这是 owner 自定义 order，不是中央槽位名（`getSectionOrder()` 只认 `SECTION_ORDERS` 里已有的键，传未知名字会得到 `undefined` 并在 `section()` 里抛「order must be a finite number」）。**为什么必须有它**：工具 schema 不等于可发现性。lazy 下模型只看到一个匿名网关 `mcp_tools` + `mcp_manager`，不知道有哪几台服务器存在，于是永远不会 `describe`，藏在 `notes` 里的环境知识（如 Blender 二进制在哪）就永远不会进入上下文——实机上表现为「AI 好像根本不知道有 MCP」。注入内容刻意保持廉价：每台服务器一行，只取 `notes` 的**首个非空行**（截断 120 字符），全局上限 700 字符；完整 `notes` 仍留在 `describe`，两级注入结构不变。`systemPrompt` **不在** `inject` 列表里，而是 `ctx.get('systemPrompt')` 可选获取——该服务缺席时插件照常挂载（工具桥接与 GUI 不受影响），回归见 `test/lifecycle.mjs` 的 D/E 段。
